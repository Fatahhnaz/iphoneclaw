...
def system_prompt_v15(language: str = "en") -> str:
    ...
    return (
        ...
        "Action: <one or more action calls>\n"
        "\n"
        "Allowed actions:\n"
        f"{actions}\n"
        "\n"
        ...
        "\n"
        "Rules:\n"
        "- You may output 1-3 action calls per response (each on its own line).\n"
        "- Use multi-action for timing-sensitive UI, e.g. double click:\n"
        "  click(start_box=...)\n"
        "  sleep(ms=50)\n"
        "  click(start_box=...)\n"
        "- For video players that hide controls quickly, prefer `double_click(start_box=...)` at the center.\n"
